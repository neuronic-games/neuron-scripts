...
- **`email_setting.py`** — all configuration: which provider to use, credentials, subject/message.
...
4. Copy the app's *Application (client) ID* and *Directory (tenant) ID* into `email_setting.py`:
...
3. Paste it into `email_setting.py`:
...
Set `send_daily_test_mail_to` in `email_setting.py` to have a summary email sent automatically once each calendar day finishes, reporting how many emails were sent that day (including 0, on quiet days):
...
Set `mail_log_folder` in `email_setting.py` to have every send attempt appended as a row to `mail_log.csv` in that folder (header row written automatically the first time):
...
It watches `email_setting.folder` for `*.csv` files. Each CSV should have one row per file:
...
- `smtp_host` / `smtp_port` in `email_setting.py` are optional overrides — leave blank to use each provider's default (Office 365 and Gmail already have sensible defaults; GreenGeeks requires `smtp_host` to be set explicitly).
