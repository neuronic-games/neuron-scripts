...
#     settings.gmail_app_password. Google no longer allows plain
...
# host is provider-specific; None means "must be supplied via settings.smtp_host"
_SMTP_DEFAULTS = {
...
def _get_provider_and_config():
    ...
    # Allow settings.smtp_host / smtp_port to override/supply the host,
    # since e.g. GreenGeeks' mail server name is account-specific.
    host_override = getattr(settings, "smtp_host", None)
    if host_override:
        cfg["host"] = host_override
    port_override = getattr(settings, "smtp_port", None)
    if port_override:
        cfg["port"] = port_override

    if not cfg["host"]:
        raise ValueError(
            "settings.smtp_host must be set when provider is %r" % provider
        )
